...
public:
	std::shared_ptr<T> GetResource(const std::string& _path, Resources* _resources, const bool _load = true, bool* _alreadyLoaded = nullptr)
	{
		// The return value
		...
		if (iterator == m_resources.end())
		{
			...
			if (_alreadyLoaded != nullptr) { *_alreadyLoaded = false; }
		}
		else
		{
			// Otherwise, return the resource as it exists.
			resource = iterator->second;
			if (_alreadyLoaded != nullptr) { *_alreadyLoaded = true; }
		}
